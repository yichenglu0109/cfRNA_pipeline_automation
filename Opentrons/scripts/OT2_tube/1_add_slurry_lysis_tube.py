"""
OT-2 tube pilot Step 1: add slurry and lysis buffer to 15 mL sample tubes.
"""
...
metadata = {
    "protocolName": "cfRNA tube pilot Step 1 - Slurry + Lysis",
    "author": "Peter Lu",
    "apiLevel": "2.13",
}
...
def run(protocol: protocol_api.ProtocolContext):
    n = sample_count()
    slurry_total = total_with_excess(SLURRY_VOL)
    lysis_total = n * LYSIS_VOL
    ...
    lysis_res = protocol.load_labware(RESERVOIR_1, SLOT_REAGENT_A)

    p300 = load_p300(protocol, [tips])
    p300.starting_tip = tips.wells()[TIP_START]

    samples = sample_wells(sample_rack)
    slurry_src = make_single_source(
        reagent_well(shared, NORG_ELU_TUBE),
        slurry_total,
        aspiration_height=SLURRY_TUBE_ASPIRATE_H,
    )
    ...
    protocol.pause(
        f"STEP 1B: Add {format_ul(lysis_total)} preheated lysis buffer + 1.2% B-ME "
        f"to the reservoir in SLOT {SLOT_REAGENT_A}. Resume to dispense lysis buffer."
    )
    ...
    protocol.comment(
        "Step 1 complete. Manually add sample to each 15 mL tube, mix thoroughly, "
        "and incubate at 60 C for 10 min before Step 3."
    )
