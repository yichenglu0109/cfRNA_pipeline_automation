"""
OT-2 tube pilot Step 4: transfer 15 mL tube contents to Norgen columns.
"""
...
try:
    ...
    TIP_START = int(os.environ.get("TIP_START", "8"))
    ...
    SAMPLE_ASPIRATE_H = float(os.environ.get("SAMPLE_ASPIRATE_H", "0.5"))
...
metadata = {
    "protocolName": "cfRNA tube pilot Step 4 - Transfer to Norgen Columns",
    "author": "Peter Lu",
    "apiLevel": "2.13",
}
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    columns = norgen_column_wells(shared)

    protocol.pause(
        f"STEP 4: Place freshly vortexed 15 mL sample tubes at SLOT {SLOT_SAMPLE_TUBES} "
        f"positions {', '.join(SAMPLE_TUBES[:n])}. "
        f"Place Norgen columns/collection tubes in SLOT {SLOT_SHARED_2ML} positions "
        f"{', '.join(NORGEN_COLUMN_TUBES[:n])}. "
        "Resume to transfer each sample to its matching column."
    )

    for index, (src, dst) in enumerate(zip(samples, columns), start=1):
        protocol.pause(
            f"Sample {index}: vortex tube, briefly spin/tap down, uncap only this tube, "
            "confirm slurry is resuspended, then resume."
        )
        p300.pick_up_tip()
        p300.mix(3, 250, src.bottom(SAMPLE_MIX_LOW_H))
        p300.mix(2, 250, src.bottom(SAMPLE_MIX_HIGH_H))
        for _ in range(3):
            p300.mix(1, 200, src.bottom(SAMPLE_MIX_LOW_H))
            p300.aspirate(230, src.bottom(SAMPLE_ASPIRATE_H), rate=0.5)
            protocol.delay(seconds=0.5)
            p300.air_gap(10)
            p300.dispense(240, dst.top(NORGEN_COLUMN_DISPENSE_FROM_TOP))
            p300.blow_out(dst.top(NORGEN_COLUMN_BLOWOUT_FROM_TOP))
        p300.drop_tip()

    protocol.comment(
        "Step 4 complete. Centrifuge Norgen columns 2 min at maximum speed or "
        "2,000 RPM, RT. Discard flow-through and proceed to Step 5."
    )
